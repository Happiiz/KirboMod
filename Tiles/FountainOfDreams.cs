    ...
    public class FountainOfDreams : ModTile
    {
        ...
        public override bool RightClick(int i, int j)
        {
                ...
                && !NPC.AnyNPCs(ModContent.NPCType<NPCs.NightmareWizard>()))
            {
                ...
                {
                    SpawnNightmareOrbAt(Main.myPlayer, i, j);
                }
                else // If the player is in multiplayer, request a spawn
                {
                    NetMethods.SpawnNightmareOrb(i, j);
                }
        ...
        internal static void SpawnNightmareOrbAt(int playerIndex, int i, int j)
        {
            Tile tile = Main.tile[i, j];
            int spawnX = i * 16 + 8;
            int spawnY = j * 16 + 16;

            //stuff to make it spawn centered regardless of which tile of the multitile you clicked
            spawnX -= tile.TileFrameX;
            spawnY -= tile.TileFrameY;
            spawnX += 52 / 2;
            spawnX += 2;
            spawnY += 72 / 2;

            NPC.NewNPC(new EntitySource_TileInteraction(Main.player[playerIndex], i, j), spawnX, spawnY, ModContent.NPCType<NightmareOrb>());
        }
    }
}
