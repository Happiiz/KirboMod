...
namespace KirboMod.Items.Weapons
{
    public class NewHammer : ModItem
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            {
                Projectile.NewProjectile(source, player.MountedCenter, new Vector2(0, 0), ModContent.ProjectileType<NewHammerHoldUp>(), 0, 0, player.whoAmI, player.itemAnimationMax);
        ...
        public override bool? UseItem(Player player)/* tModPorter Suggestion: Return null instead of false */
        {
            if (uses >= 3 & player.velocity.Y == 0 && player.altFunctionUse != 2)
            {
                ...
                Projectile.NewProjectile(new EntitySource_ItemUse(Main.player[player.whoAmI], player.HeldItem), player.Center.X + player.direction * 40, player.Center.Y,
                        player.direction * 8, 0, ModContent.ProjectileType<Projectiles.NewHammerShockwave>(), (int)(player.GetWeaponDamage(Item) * 1.5f), player.GetWeaponKnockback(Item) * 2, player.whoAmI);
