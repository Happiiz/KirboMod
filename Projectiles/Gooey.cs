                ...
                else
                {
                    ...
                    if (flying == true)
                    {
                        if (attack == 0) //if not attacking
                        {
                            float speed = 18f;
                            float inertia = 9f;
                            //math I pulled out of thin air to attempt to make gooey's movement less akwawrd when the target is stationary
                            float extraAngleOffset = Utils.Remap(Projectile.ai[1] * ((0.05f * (Projectile.identity + 1)) % 1), 0, 1, -0.2f, 0.2f, false);
                            direction = (aggroTarget.Center + ((float)Projectile.identity + extraAngleOffset).ToRotationVector2() * 100) - Projectile.Center; //start - end
                            ...
                            {
                                SoundEngine.PlaySound(PureDarkMatter.LaserSFX.WithVolumeScale(0.5f).WithPitchOffset(0.5f), Projectile.Center);
                                if (Main.myPlayer == Projectile.owner)
                                {
                                    int projType = ModContent.ProjectileType<GooeyDarkMatterLaser>();
                                    float shootSpeed = 32f;
                                    float effectiveShootSpeed = shootSpeed * ContentSamples.ProjectilesByType[projType].MaxUpdates;
                                    ...
                                    if (aggroTarget != null)
                                    {
                                        ...
                                        {
                                            direction = results.ChaserVelocity;
                                        }
                                        else
                                        {
                                            direction = Vector2.Normalize((aggroTarget.Center + aggroTarget.velocity * (Vector2.Distance(Projectile.Center, aggroTarget.Center) / effectiveShootSpeed)) - Projectile.Center);
                                        }
                                    }
                                    ...
                                    Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, direction,
                                       projType, Projectile.damage, 3, Projectile.owner, ai0, ai1); //fire good laser
                                }
                            }
                        }
                    }
                    else //walk
                    {
                        ...
                        if (attack == 0) //if not attacking
                        {
                            ...
                            Vector2 carrotDirection = new(pseudoDirection, 0); //start - end 
            ...
            if (attacktype == 1) //fire dash
            {
                ...
                if (attack >= 1) //during attack
                {
                    ...
                    if (attack == 1)
                    {
                        ...
                        {
                            float dashSpeed = 40;
                            Utils.ChaseResults results = Utils.GetChaseResults(Projectile.Center, dashSpeed, aggroTarget.Center, aggroTarget.velocity);
                            if (results.InterceptionHappens)
                            {
                            ...
                            {
                                Projectile.velocity = Vector2.Normalize((aggroTarget.Center + aggroTarget.velocity * (Vector2.Distance(Projectile.Center, aggroTarget.Center) / dashSpeed)) - Projectile.Center);
                            }
                        }
