...
{
	public class DuoChillyMinion : ModProjectile
	{
		int attack = 0;
		...
		public override void AI()
		{
			...
			{
				player.ClearBuff(ModContent.BuffType<Buffs.MinionBuffs.LeoAndChillyBuff>());
			}
			if (player.HasBuff(ModContent.BuffType<Buffs.MinionBuffs.LeoAndChillyBuff>()))
			{
		...
		private void Attack()
        {
			...
			/*if (attack % 2 == 0)
			{
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, projshoot.RotatedByRandom(MathHelper.ToRadians(360)), Mod.Find<ModProjectile>("MinionIce").Type, Projectile.damage, 1, Main.myPlayer, 0, 0);
			}

            if (attack % 10 == 0) //every 10th tick
            {
                SoundEngine.PlaySound(SoundID.Item24, Projectile.Center); //spectre boots
            }*/

			if (attack == 1)
			{
                Player player = Main.player[Projectile.owner];
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity *= 0.01f, Mod.Find<ModProjectile>("ChillyMinionFreeze").Type, Projectile.damage, 1, player.whoAmI, 0, Projectile.whoAmI);
		...
		public override bool TileCollideStyle(ref int width, ref int height, ref bool fallThrough, ref Vector2 hitboxCenterFrac)
		{
            ...
            if (aggroTarget != null && aggroTarget.active && !aggroTarget.dontTakeDamage) //fall to enemy
            {
				...
				if (toTarget.Y > 1 && Math.Abs(toTarget.X) < 300)
				{
					fallThrough = true;
				}
				else
