...
namespace KirboMod.Projectiles.NightmareLightningOrb
{
    public class NightmareLightningOrb : ModProjectile//180x180 proj? use circle collision ofc
    {
        ...
        static float LaunchVelocity => 20;
        static float OrbInitialVelocityLength => 50;
        float OrbSpecificLaunchDelay => Projectile.ai[0];
        static float ExtraLaunchDelay => 60;
        ...
        public static void GetShootStats(int start, int fireRate, int timer, int totalOrbs, int targetPlayerIndex, out float ai0, out float ai1, out float ai2, out Vector2 velocity)
        {
            float progressOnAttack = Utils.GetLerpValue(start, start + fireRate * totalOrbs, timer);
            velocity = new Vector2(0, OrbInitialVelocityLength).RotatedBy(progressOnAttack * MathF.Tau);
            ai0 = MathHelper.Lerp(0, fireRate * totalOrbs, progressOnAttack);
            ai1 = targetPlayerIndex;
            ai2 = progressOnAttack;
        }
        ...
        public override void AI()
        {
            ...
            if (Projectile.alpha < 0)
                Projectile.alpha = 0;
            if (Timer >= ScaleUpTime && Timer <= ScaleUpTime + ExtraLaunchDelay + OrbSpecificLaunchDelay)
            {
                float extraProgress = Utils.GetLerpValue(0, 100, Timer - ScaleUpTime);
                Projectile.Center = LaunchFrom + new Vector2(0, (OrbRadius * 4) * extraProgress).RotatedBy((OrbInitialProgressOnAttack + extraProgress) * MathF.Tau);
            }
            else if (Timer == ScaleUpTime + ExtraLaunchDelay + OrbSpecificLaunchDelay + 1)
            {
            ...
            Projectile.Center = LaunchFrom - Projectile.velocity;
            Timer++;
        }
        ...
        //majorly purple, sometimes very dark blue, sometimes dark red
        //has some sparks randomly inside the sphere somewhat similar to tesla orb arcs or whatever
        //inner glowy white core
        //glowy white core emits electric arcs that scale up
        //orb seems to have some kind of faint smoke inside it?
        //outline also frequently flashes colors
        ...
        {
            //this is a slow attack with an effectively very big hitbox, so makes sense it'd hit heavy
            target.AddBuff(BuffID.Blackout, 60 * 10, false);
            target.AddBuff(BuffID.Slow, 60 * 3);//ankh shields gives immunity. maybe make dedicated debuff instead?
            target.AddBuff(BuffID.OgreSpit, 60, false);
            target.AddBuff(BuffID.Electrified, 60 * 10);//reduce damage of attack to compensate, but don't reduce too much
        }
        public override bool ShouldUpdatePosition() => Timer >= ScaleUpTime + ExtraLaunchDelay + OrbSpecificLaunchDelay;
    }
}
