    ...
    public abstract class LightningProj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.velocity == Vector2.Zero)
            {
                ...
                if (Main.rand.NextBool(Projectile.MaxUpdates * 2))
                {
                    for (int i = 0; i < 2; i++)
                    {
            ...
            for (int i = 0; i < 3; i++)
            {
                switch (i)
                {
                    ...
                    case 1:
                        ...
                        DelegateMethods.c_1 = Color.Lerp(outerColor, innerColor, 0.5f) * 0.5f;
                        break;
                    default:
                ...
                for (int j = Projectile.oldPos.Length - 1; j > 0; j -= Projectile.MaxUpdates)
                {
                    ...
                    Vector2 end = Projectile.oldPos[j - Projectile.MaxUpdates] + new Vector2(Projectile.width, Projectile.height) / 2f + Vector2.UnitY * Projectile.gfxOffY - Main.screenPosition;
                ...
                DelegateMethods.f_1 = 1f;
                Vector2 lastPos = Projectile.oldPos[Projectile.MaxUpdates] + new Vector2(Projectile.width, Projectile.height) / 2f + Vector2.UnitY * Projectile.gfxOffY - Main.screenPosition;
        ...
        public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
        {
            for (int i = Projectile.oldPos.Length - 1; i >= 0; i--)
            {
                if (Projectile.oldPos[i] == Vector2.Zero)
                    continue;
                Rectangle hitbox = new Rectangle((int)Projectile.oldPos[0].X, (int)Projectile.oldPos[0].Y, Projectile.width, Projectile.height);
