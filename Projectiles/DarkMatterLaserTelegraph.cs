...
{
	public class DarkMatterLaserTelegraph : ModProjectile
	{
		...
		{
			NPC darkmatter = Main.npc[(int)Projectile.ai[1]];

			//go to eye position
            Vector2 turned = darkmatter.direction == 1 ? darkmatter.rotation.ToRotationVector2() : darkmatter.rotation.ToRotationVector2().RotatedBy(MathHelper.ToRadians(180));

            initalposition = darkmatter.Center + turned * 45;
